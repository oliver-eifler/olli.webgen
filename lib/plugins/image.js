...
(function() {
    ...
    function processImage(dom,node,options) {
        ...
        //style it
        if (!attr.zoom) {
                if (node.parent().is("figure"))
                    node.parent().css("max-width", attr.width + "px");
        ...
        if (attr.frame) {
            attr.async = true;
            image = dom("<iframe scrolling='no' data-type='img' allowtransparency='true' frameborder='0' width='"+attr.width+"px' height='"+attr.height+"px'></iframe>");
        }
        else {
            image = dom('<img>');
        }
        ...
        //and now the caption
        var caption = (attr.caption.trim()=="")?attr.src:attr.caption;
        box.append("<div class='caption'><a href='"+attr.src+"' class='caption-text'>" + caption + "</a></div>");
