    ...
    if (_.isString(path) && path != "") {
        ...
        try {
            ...
            this.options.data = _.extend($this.options.data,{
                created: fileStat.birthtime.getTime(),
...
WebGen.prototype.save = function (path) {
    if (this.dom && _.isString(path) && path != "") {
        ...
        var json =_.extend({},this.options.data);
        json.content = this.dom.html();
        path+=".json";
        try {
            fs.writeFileSync(path, JSON.stringify(json), this.options.encoding);
            console.log(" => saved to %s", path);
        } catch (err) {
            console.log(" => %s", err.message);
        }
